		...
		for (var i=0; i<prs.length; i++) {
			...
			if(pr.pullrequests != undefined) {
				for(var j=0; j<pr.pullrequests.length; j++) {
					...
					var paramPrefix = pr.repository + "." + req.id;
					table.fnAddData( [ 
		                        '<input type="checkbox" name="' + paramPrefix + '.selected" id="checkall" checked="checked">',
		                        '#' + req.id + '<input type="hidden" name="' + paramPrefix + '.idx" value="' + j + '"/>',
		                        ...
		                        '<span id="status_' +  j + '" class="status" ></span>' +
		                        '<label id="status_description_' + j + '" class="synch-status">'
			                  ] );
				}
