$(function() {
	...
	var refresh = function() {
		$.post('teams-import-status.gh', function(data) {
			var teams = eval('(' + data + ')');
			...
			if(teams.length == 0) {
				...
				for (var i=0; i<teams.length; i++) {
					var id = teams[i].index;
					var value = teams[i].value;
					var status = teams[i].status.toLowerCase();
			...
			if(allCompleted || teams.length <= 0) {
				completed = true;
