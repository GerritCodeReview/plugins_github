...
public class GitHubOrganisationGroup implements Basic {
  ...
  public static Set<GroupReference> listByPrefix(
      UserScopedProvider<GitHubLogin> ghLoginProvider, String orgNamePrefix) {
    try {
      String orgNamePrefixLowercase = orgNamePrefix.toLowerCase();
      Set<String> ghOrgs = ghLoginProvider.get().getMyOrganisationsLogins();
      ...
      for (String ghOrg : ghOrgs) {
        if (ghOrg.toLowerCase().startsWith(orgNamePrefixLowercase)) {
          orgGroups.add(new GroupReference(uuid(ghOrg), NAME_PREFIX + ghOrg));
        }
      }
  ...
  public static Set<UUID> listByUser(
      UserScopedProvider<GitHubLogin> ghLoginProvider, String username) {
    try {
      Set<String> ghOrgsLogins =
          ghLoginProvider.get(username).getMyOrganisationsLogins();
      ImmutableSet.Builder<UUID> groupSet = new ImmutableSet.Builder<UUID>();
      for (String ghOrg : ghOrgsLogins) {
        groupSet.add(uuid(ghOrg));
      }
