...
public class GitHubUser {
  ...
  private GitHubUser(GHUser gitHubUser, GitUser author) throws IOException {
    this.login = getLogin(gitHubUser).or(generateLogin(author.getName()));
    this.name = getFullName(gitHubUser).or(author.getName());
    this.email = getEMail(gitHubUser).or(author.getEmail());
  }

  private static Optional<String> getLogin(GHUser gitHubUser) {
    return Optional.fromNullable(gitHubUser != null ? gitHubUser.getLogin()
        : null);
  }

  private static Optional<String> getEMail(GHUser gitHubUser)
      throws IOException {
    return Optional.fromNullable(gitHubUser != null ? Strings
        .emptyToNull(gitHubUser.getEmail()) : null);
  }

  private static Optional<String> getFullName(GHUser gitHubUser)
      throws IOException {
    return Optional.fromNullable(gitHubUser != null ? Strings
