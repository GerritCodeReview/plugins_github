...
public class GitHubGroupCacheRefreshFilter extends AllRequestFilter {
  ...
  private static final String REFRESH_GROUPS_ATTRIBUTE = "RefreshGroups";
  ...
  public void doFilter(
    ...
    if (req.getRequestURI().endsWith(LOGIN_URL) && req.getQueryString().equals(LOGIN_QUERY_FINAL)) {
      ...
      if (cookieResponse != null && cookieResponse.contains(ACCOUNT_COOKIE)) {
        req.getSession().setAttribute(REFRESH_GROUPS_ATTRIBUTE, Boolean.TRUE);
      }
    } else if (Optional.ofNullable(req.getSession(false))
        .flatMap(session -> Optional.ofNullable(session.getAttribute(REFRESH_GROUPS_ATTRIBUTE)))
        .filter(refresh -> (Boolean) refresh)
        .isPresent()) {
      ghGroupsCache.refreshCurrentUserGroups();
      req.getSession().removeAttribute(REFRESH_GROUPS_ATTRIBUTE);
    }
  }
