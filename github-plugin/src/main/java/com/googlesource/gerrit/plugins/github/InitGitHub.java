...
public class InitGitHub implements InitStep {
  ...
  private void setupGitHubOAuthTokenCipher() {
    ...
    if (maybeCurrentKeyId.isPresent()) {
      if (!ui.yesno(
          false,
          "Current GitHub OAuth token cipher is configured under %s key id. Do you want to configure new one?",
      ...
      do {
        ...
        if (cfg.getSubsections(CONF_KEY_SECTION).contains(newKeyId)) {
          notUniqueKeyIdPrefix =
              String.format(
                  "Provided key id '%s' already exists. Please provide different k", newKeyId);
          continue;
        }
    ...
    if (defaultPasswordPath.toString().equalsIgnoreCase(currentPasswordPath)) {
      ...
      new PasswordGenerator().generate(Path.of(currentPasswordPath));
      if (ui.yesno(
          true,
          "Default password file was selected '%s'. Do you want to rely on Gerrit defaults?",
          currentPasswordPath)) {
        return;
      }
