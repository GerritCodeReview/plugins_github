load("//tools/bzl:maven_jar.bzl", "MAVEN_LOCAL", "maven_jar")

def external_plugin_deps():
   maven_jar(
        name = "javax-servlet-api",
        artifact = "javax.servlet:javax.servlet-api:3.0.1",
        sha1 = "6bf0ebb7efd993e222fc1112377b5e92a13b38dd",
    )

    maven_jar(
        name = "slf4j-api",
        artifact = "org.slf4j:slf4j-api:1.7.5",
        sha1 = "6b262da268f8ad9eff941b25503a9198f0a0ac93",
    )

    maven_jar(
        name = "slf4j-log4j12",
        artifact = "org.slf4j:slf4j-log4j12:1.7.5",
        sha1 = "6edffc576ce104ec769d954618764f39f0f0f10d",
    )

    maven_jar(
        name = "github-api",
        artifact = "org.kohsuke:github-api:1.69-SNAPSHOT",
        repository = MAVEN_LOCAL,
        attach_source = False,
    )

    maven_jar(
        name = "lombok",
        artifact = "org.projectlombok:lombok:1.18.4",
        sha1 = "7103ab519b1cdbb0642ad4eaf1db209d905d0f96",
    )

    maven_jar(
        name = "bridge-method-annotation",
        artifact = "com.infradna.tool:bridge-method-annotation:1.14",
        sha1 = "0e65d67120c0c28425ca606c57aef19786717a28",
    )
