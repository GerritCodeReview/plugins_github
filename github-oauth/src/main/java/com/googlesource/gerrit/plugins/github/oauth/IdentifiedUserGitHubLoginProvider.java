...
public class IdentifiedUserGitHubLoginProvider implements
  ...
  public GitHubLogin get(String username) {
    try {
      ...
      login.login(newAccessTokenFromUser(username));
      return login;
    } catch (IOException e) {
      throw new IllegalStateException("Cannot login to GitHub as '" + username
          + "'", e);
    }
  }
  ...
  private AccessToken newAccessTokenFromUser(String username) {
    ...
    throw new IllegalStateException("Current user '" + username
        + "' has not a GitHub OAuth external id associated");
  }
}
