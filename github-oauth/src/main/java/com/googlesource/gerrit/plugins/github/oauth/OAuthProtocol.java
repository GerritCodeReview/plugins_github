...
public class OAuthProtocol {
  ...
  public String newRandomState(HttpServletRequest request) throws UnsupportedEncodingException {
    ...
    Optional<String> finalRedirectUrl =
        Optional.ofNullable(request.getParameter(OAuthWebFilter.FINAL_REDIRECT_PARAMETER));
    if (finalRedirectUrl.isPresent()) {
      return state
          + ME_SEPARATOR
          + OAuthWebFilter.FINAL_REDIRECT_PARAMETER
          + "="
          + URLEncoder.encode(finalRedirectUrl.get(), "UTF-8");
    }
    return state;
  }
  ...
  public static String getTargetUrl(ServletRequest request) {
    ...
    if (subStrings.length == 2) {
      String requestURI = subStrings[1];
      return requestURI + firstQueryParameter;
    }
    if (subStrings.length == 3) {
      String requestURI = subStrings[1];
      String secondQueryParameter = subStrings[2];
      return requestURI + firstQueryParameter + "&" + secondQueryParameter;
    }
