...
public class OAuthTokenCipher {
  ...
  public String decrypt(final String base64EncryptedString) throws CipherException {
    try {
      ...
      if (isPrefixedWithKeyId(base64EncryptedString)) {
        ...
        return new String(
            decryptCipher.doFinal(Base64.getDecoder().decode(dropKeyId(base64EncryptedString))),
  ...
  private String prependCurrentKeyId(String base64EncodedString) {
    return String.format("%s:%s", currentKeyId, base64EncodedString);
  }

  private boolean isPrefixedWithKeyId(String maybeEncryptedString) {
    return maybeEncryptedString.contains(":");
  }

  private static String extractFromString(String base64EncryptedString, int idx) {
    List<String> tokens = Splitter.on(':').splitToList(base64EncryptedString);
  ...
  private Cipher getCipherFor(String base64EncryptedString) throws IOException {
    String prefix = takeKeyId(base64EncryptedString);
    GitHubOAuthConfig.KeyConfig keyConfig = config.getKeyConfig(prefix);
    if (keyConfig == null) {
      throw new IllegalStateException(
          String.format("Could not find key-id '%s' in configuration", prefix));
    }
    return initCipher(
