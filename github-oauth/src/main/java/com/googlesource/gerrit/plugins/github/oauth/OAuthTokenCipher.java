...
public class OAuthTokenCipher {
  ...
  private static String extractFromString(String base64EncryptedString, int idx) {
    ...
    if (nOfTokens != 2) {
      throw new IllegalStateException(
          String.format("malformed encrypted token %s found, expected 2", nOfTokens));
    }
    return tokens[idx];
  }
  ...
  private Cipher getCipherFor(String base64EncryptedString) throws IOException {
    GitHubOAuthConfig.KeyConfig keyConfig = config.getKeyConfig(takePrefix(base64EncryptedString));
    if (keyConfig == null) {
      throw new IllegalStateException(
          String.format(
              "Could not find prefix '%s' in configuration", takePrefix(base64EncryptedString)));
    }
    return initCipher(
