...
public class GitHubOAuthConfig {
  ...
  @Getter public final Map<String, Map<ScopeKey, List<OAuthProtocol.Scope>>> virtualScopes;
  ...
  private Map<String, Map<ScopeKey, List<Scope>>> getVirtualScopes(Config config) {
    return config.getSubsections(CONF_SECTION).stream()
        .collect(Collectors.toMap(k -> k, v -> getScopesInSection(config, v)));
  }

  private Map<ScopeKey, List<Scope>> getScopesInSection(Config config, String subsection) {
