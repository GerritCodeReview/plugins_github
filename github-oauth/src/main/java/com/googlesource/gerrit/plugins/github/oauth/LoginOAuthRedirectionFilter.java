...
public class LoginOAuthRedirectionFilter implements Filter {
  ...
  private static final Logger log = LoggerFactory.getLogger(LoginOAuthRedirectionFilter.class);
  ...
  public void doFilter(
    ...
    Optional<String> forwardedHost =
        extractClientHost(httpRequest.getHeader(HttpHeaders.X_FORWARDED_HOST));
    ...
    if (forwardedHost.isPresent()
        && !canonicalWebUrl.getHost().equalsIgnoreCase(forwardedHost.get())
        && !Strings.isNullOrEmpty(forwardedPort)
        && !Strings.isNullOrEmpty(forwardedProto)) {
      String loginURL =
          buildLoginURLWithFinalRedirectURL(
              forwardedProto, forwardedHost.get(), Integer.parseInt(forwardedPort));
      log.info("Redirect from URL: {} to URL: {}", httpRequest.getRequestURL(), loginURL);
      httpResponse.sendRedirect(loginURL);
      return;
    }

    log.error(
        "The X-Forwarded-Host header: {}, X-Forwarded-Port header: {} and X-Forwarded-Proto header: {} are "
            + "compulsory and X-Forwarded-Host must be different than canonical web url host: {}",
        forwardedHost,
        forwardedPort,
        forwardedProto,
        canonicalWebUrl.getHost());
